...
<div class="container-fluid">
			<div class="col-xs-6 col-sm-6 col-md-10">
				<br><br>
				<center><h1>TOUTES LES IEP</h1></center>
			

<nav class="navbar navbar-inverse">
    <ul class="nav navbar-nav">
        <li><a href="{{ URL::to('iep') }}">Voir les iep</a></li>
        <li><a href="{{ URL::to('iep/create') }}">Creer une nouvelle iep</a>
    </ul>
</nav>
    ...
    @foreach($iep as $key => $item)
     ...
     <td>{{ $item->DrenID }}</td>
      ...
      <!-- we will also add show, edit, and delete buttons -->
            <td>
               <a class="btn btn-small btn-success" href="{{ URL::to('iep/' . $item->ID) }}">VOIR</a>

                <!-- edit this nerd (uses the edit method found at GET /nerds/{id}/edit -->
                <a class="btn btn-small btn-info" href="{{ URL::to( 'iep/' .$item->ID. '/edit' ) }}">MODIFIER</a>

                <!-- bouton de suppression -->
                {{ Form::open(array('url' => 'iep/' . $item->ID, 'class' => 'pull-right')) }}
